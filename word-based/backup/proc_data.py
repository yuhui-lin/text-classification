...
def load_data_and_labels(dataset=FLAGS.dataset, data_dir=FLAGS.datasets_dir):
    ...
    x_train_alian = pad_sentences(x_train, maxlen)
    ...
    x_test_alian = pad_sentences(x_test, maxlen)
...
def main(argv):
    ...
    convert_to(np.array(x_train, dtype=np.int64), [a.index(1) for a in y_train], 'train')
    convert_to(np.array(x_test, dtype=np.int64), [a.index(1) for a in y_test], 'test')
