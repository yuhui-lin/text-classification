...
NEWSGROUPS_DICT = {"comp.graphics": "comp", "comp.os.ms-windows.misc": "comp",
				   "comp.sys.ibm.pc.hardware": "comp", "comp.sys.mac.hardware": "comp",
				   "comp.windows.x": "comp", "talk.politics.misc": "politics", "talk.politics.guns": "politics",
				   "talk.politics.mideast": "politics", "rec.autos": "rec", "rec.motorcycles": "rec", "rec.sport.baseball": "rec",
				   "rec.sport.hockey": "rec", "talk.religion.misc": "religion", "alt.atheism": "religion",
...
def maybe_download(data_dir, source_name, source_downloaded, source_url):
	...
	if not tf.gfile.Exists(source_path):
		...
		download_path, _ = urllib.urlretrieve(
...
def grab_data_from_folder(source_path, selected_folders, str_filter=lambda x: x, folder_map=lambda x: x):
			...
			if line.startswith("<source>"):
				...
				if cat in AG_CATEGORIES and count[cat] < AG_CATEGORIES_NUM:
					...
					if title and desc:
						...
						labels.append(
							create_one_hot_vector(cat, AG_CATEGORIES))
						# print(sequ)
						# print("-------------")
	return [sequences, labels]
...
def grab_data_rotten(source_path):
	pos_path = os.path.join(
		source_path, "rt-polaritydata/rt-polarity.pos")
	neg_path = os.path.join(
		source_path, "rt-polaritydata/rt-polarity.neg")
...
def raw_data_statistics(data_name, sequences, labels):
	...
	print("the average length of data examples:", sum(len(s)
...
def load_data_and_labels(dataset, data_dir):
	...
	if dataset == "rotten":
		source_path = maybe_download(
			data_dir, ROTTEN_SOURCE, ROTTEN_DOWNLOADED, ROTTEN_URL)
		# Load data from files
		print ("current working directory:", os.getcwd())
		sequences, labels = grab_data_rotten(source_path)
		print("shuffling dataset and splitting train/test sets")
		x_train, y_train, x_test, y_test = shuffle_and_split(
			sequences, labels, test_ratio)

	elif dataset == "ag":
		source_path = maybe_download(
			data_dir, AG_SOURCE, AG_DOWNLOADED, AG_URL)
		...
		source_path = maybe_download(
			data_dir, NEWSGROUPS_SOURCE, NEWSGROUPS_DOWNLOADED, NEWSGROUPS_URL)

		print("load train set")
		train_path = os.path.join(source_path, "20news-bydate-train")
		x_train, y_train = grab_data_from_folder(
			train_path, NEWSGROUPS_CATEGORIES, folder_map=lambda x: NEWSGROUPS_DICT.get(x))
		shuffle_data(x_train, y_train)
		print("load test set")
		test_path = os.path.join(source_path, "20news-bydate-test")
		x_test, y_test = grab_data_from_folder(
			test_path, NEWSGROUPS_CATEGORIES, folder_map=lambda x: NEWSGROUPS_DICT.get(x))
		shuffle_data(x_test, y_test)

	elif dataset == "imdb":
		source_path = maybe_download(
			data_dir, IMDB_SOURCE, IMDB_DOWNLOADED, IMDB_URL)
		print("load train set")
...
def pad_sentences(sentences, padding_word="<PAD/>", max_length=-1):
	"""
...
def align_sequences(sequences, padding_character=' ', max_length=-1):
	...
	for sequence in sequences:
		if not isinstance(sequence, basestring):
			...
			print(sequence)
			print("\n\n")
		new_sequence = sequence.ljust(
			max_length, padding_character)[:max_length]
